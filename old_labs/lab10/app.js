...
file_system.writeFileSync('datos.txt', 'Aqui se almacena informacion de personajes')
...
const server = http.createServer( (request, response) => {
    if (request.url == '/' && request.method == "GET"){
        ...
        response.write('<p>Bienvenido</p>');
        response.write(html_footer);
        response.end();
    } else if (request.url === '/about' && request.method === 'GET'){
        response.setHeader('Content-Type', 'text/html');
        response.write(html_header);
        response.write('<p>Acerca de esta aplicación</p>');
        response.write(html_footer);
        response.end();
      ...
      request.on('end', () => {
        ...
        const nombre = datos_completos.split('=')[1];
            ...
            file_system.appendFile('datos.txt', nombre + '\n', (err) => {
